...
Server Actions marked with 'use server' run on the server and are callable from the client.
...
use() also unwraps context, an alternative to useContext.
...
Stream components as they resolve for a faster initial page load.

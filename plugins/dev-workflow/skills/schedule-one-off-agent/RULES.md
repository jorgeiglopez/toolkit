...
- Always disclose the caveats to the user: session-only (not persisted to disk), dies if the session closes before fire time, only fires while the session is idle, not a real OS cronjob.

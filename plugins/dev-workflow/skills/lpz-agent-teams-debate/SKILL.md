...
> Using the `lpz-agent-teams-debate` skill to debate <topic>.
...
- Compute the reports directory and create it:
  ```bash
  REPORT_DIR="/tmp/lpz-agent-teams-debate/$TEAM/reports"
  mkdir -p "$REPORT_DIR"
  ```
...
If any file is missing after a reasonable wait, re-prompt that teammate ONCE with the missing path. If still missing, fall back to `lpz-agent-recall-exec` for that teammate only.
...
The reports under `/tmp/lpz-agent-teams-debate/$TEAM/` survive `TeamDelete` — tell the user the path in case they want the raw inputs.
...
- No commits, no edits to user code. This skill is read-only on the repo, write-only to `/tmp/lpz-agent-teams-debate/`.

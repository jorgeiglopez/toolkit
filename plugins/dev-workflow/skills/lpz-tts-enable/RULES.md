...
- Volume is a decimal linear gain in `~/.claude/toolkit/tts/tts-volume`, default 1.8 — calibrated to lift Kokoro-82M's quiet WAVs to roughly `say`'s level (applied via `afplay -v`, clamped 0.1–4.0). `say` speaks through the system synthesizer with no gain hook and ignores it. Read per message, no restart.
